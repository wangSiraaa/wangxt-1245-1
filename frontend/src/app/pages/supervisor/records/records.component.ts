...
@Component({
  selector: 'app-supervisor-records',
  standalone: true,
  imports: [
  ...
  template: `
    <h2 class="page-title">全量发运记录</h2>

    <mat-card class="filter-card">
      <div class="filter-row">
        <mat-form-field appearance="outline" class="filter-field">
          <mat-label>搜索</mat-label>
          <input matInput [(ngModel)]="keyword" (keyup.enter)="load()" placeholder="单号/品名/车牌/货主">
        </mat-form-field>
        <mat-form-field appearance="outline" class="filter-field">
    ...
    <mat-card>
      <mat-table [dataSource]="data()" class="table">
        ...
        <ng-container matColumnDef="freight">
          <mat-header-cell *matHeaderCellDef>货运员</mat-header-cell>
          <mat-cell *matCellDef="let r">{{ r.freightInspector?.name || '-' }}</mat-cell>
        </ng-container>
...
export class SupervisorRecordsComponent implements OnInit {
  private api = inject(ApiService);
  data = signal<ShipmentRecord[]>([]);
  keyword = '';
  filterStatus = '';
  columns = ['shipmentNo', 'goodName', 'owner', 'vehicle', 'status', 'freight', 'createdAt', 'actions'];
